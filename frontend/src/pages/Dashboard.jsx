...
const Dashboard = () => {
  ...
  const [ BesnisesStats, setBesnisesStats ] = useState([]);
    ...
    const fetchGeneralData = async() => {
        ...
        if(data.user.role === 'user') {
             const activityStats = async() => {
                const res = await fetch('http://127.0.0.1:8000/api/statistics/activities/2', {
                    method: 'GET',
                    headers: {
                        'Accept' : 'application/json',
                        'Content-Type' : 'application/json',
                ...
                const Actstats = await res.json();
                setActivityStats(Actstats)
            }
            activityStats();
        } else if (data.user.role = 'orgnizer'){
            const businesseStats = async() => {
                const res = await fetch('http://127.0.0.1:8000/api/statistics/businesses/1', {
                    method: 'GET',
                    headers: {
                        'Accept' : 'application/json',
                        'Content-Type' : 'application/json',
                ...
                const BusnStats = await res.json();
                setBesnisesStats(BusnStats);
            }
